...
var proxy = new JsonRpcProxy('http://bach/hub2/TestContext/Calculator',['keyPress'])
displayResult = function (result) {
    inner = $('display');
    inner.innerHTML = repr(result);
}
...
var doKeyPress = function (key,evt) {
    var r = proxy.keyPress(key);
    r.addCallback(displayResult);
    r.addErrback(displayError);
}
                                                                                                                                                 
var addAction = function (mybutton) {
    addToCallStack(mybutton,'onclick',partial(doKeyPress,mybutton.id));
}
                                                                                                                                                 
pageinit = function () {
    var buttons = getElementsByTagAndClassName('button',null);
    map(addAction,buttons);
}
                                                                                                                                                 
MochiKit.DOM.addLoadEvent(pageinit)


Description
===========

JsonRpc provides basic facilities to do an asyncronous request to
an JsonRpc Server. (see json-rpc.org).
In order to do this, JavaScript objects can be converted into
JSON strings (at the moment: numbers, strings, Array like and general object).
The conversion is done by the 'json' function which works just as 'repr'.
The module provides an 'JsonRpcProxy'. A call to a proxy method returns a Deferred!
Since JavaScript doesn't seem to have special __getattr__, __setattr__, etc. methods,
all proxy methods must be declared (or just use the more generic 'call' method.
...
MochiKit.JsonRpc.JsonRpcError
-----------------------------

Constructors
++++++++++++

MochiKit.JsonRpc.JsonRpcCall(method,params)
-------------------------------------------

MochiKit.JsonRpc.JsonRpcProxy(url [, methodNames])
--------------------------------------------------

Functions
+++++++++

MochiKit.JsonRpc.jsonObject(obj)
--------------------------------

MochiKit.JsonRpc.isObject(obj)
------------------------------

MochiKit.JsonRpc.jsonArray(obj)
-------------------------------

MochiKit.JsonRpc.json(obj)
--------------------------

MochiKit.JsonRpc.DomObjectFromJson(???)
---------------------------------------
*/
...
MochiKit.Base.update(MochiKit.JsonRpc.JsonRpcProxy.prototype, {
    'call': function () {
