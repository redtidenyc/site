...
from rt_www.admin.filterspecs import FilterSpec
...
def render_change_form(model, context, add=False, change=False, form_url=''):
	# opts = model._meta
	app_label = model._meta.app_label
	auto_populated_fields = [f for f in model._meta.fields]
	# field_sets = model._meta.admin.get_field_sets(model._meta)
	# original = getattr('original_object', None)
	...
	return render_to_response([
		"admin/%s/%s/change_form.html" % (app_label, model._meta.object_name.lower()),
		"admin/%s/change_form.html" % app_label,
		"admin/change_form.html"], context_instance=context)

def index(request):
	return render_to_response('admin/index.html', {'title': _('Redtide Administration')}, 
...
def add_stage(request, app_label, model_name, show_delete=False, form_url='', post_url=None, post_url_continue='../%s/', object_id_override=None):
	...
	form = forms.ModelForm(manipulator)
	...
	return render_change_form(model, c, add=True)
...
def change_stage(request, app_label, model_name, object_id):
	...
	try:
		manipulator = model.objects.get(id=object_id)
	...
	if request.POST:
		...
		new_data = manipulator.value_to_string()
		...
		for rel_obj in opts.get_ordered_objects():
			id_order_list.extend(getattr(original_object, 'get_%s_order' % rel_obj.object_name.lower())())
		if id_order_list:
	...
	"""for related in opts.get_followed_related_objects():
		...
		if wrt and wrt.rel and wrt.rel.to == opts:
			func = getattr(original_object, 'get_%s_list' %
